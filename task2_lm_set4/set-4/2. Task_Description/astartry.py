...
print(stop)
'''
for i in p:#adds neighbours of everything except obstacles
    x=i[0]
    y=i[1]
    if p[i][0] != '1':
        p[i][1] = lookup(x,y-1)#north
        p[i][2] = lookup(x-1,y)#west
        p[i][3] = lookup(x,y+1)#south
        p[i][4] = lookup(x+1,y)#east
